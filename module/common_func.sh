...
download() { busybox wget -T 3 --no-check-certificate -qO - "$1" > "$2" || busybox wget -T 3 --no-check-certificate -qO - "${PRX}$1" > "$2" || download_fail "$1"; }
if command -v curl > /dev/null 2>&1; then
    download() { curl --connect-timeout 3 -s "$1" > "$2" || curl --connect-timeout 3 -s "${PRX}$1" > "$2" || download_fail "$1"; }
fi
